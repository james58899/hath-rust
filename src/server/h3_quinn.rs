// Copy from https://github.com/hyperium/h3/blob/e523eaf3f0dc8a8e0df4cd10f71c9d00809ea666/h3-quinn/src/lib.rs
// Add call stopped() in poll_finish() to wait peer receive all data
...
impl Connection {
    ...
    pub fn new(conn: quinn::Connection) -> Self {
        Self {
            conn: conn.clone(),
            incoming_bi: Box::pin(stream::unfold(conn.clone(), |conn| async { Some((conn.accept_bi().await, conn)) })),
            opening_bi: None,
            incoming_uni: Box::pin(stream::unfold(conn.clone(), |conn| async { Some((conn.accept_uni().await, conn)) })),
            opening_uni: None,
        }
    }
}
...
fn convert_connection_error(e: quinn::ConnectionError) -> h3::quic::ConnectionErrorIncoming {
    match e {
        quinn::ConnectionError::ApplicationClosed(application_close) => ConnectionErrorIncoming::ApplicationClose {
        ...
        | error @ quinn::ConnectionError::ConnectionClosed(_) => ConnectionErrorIncoming::Undefined(Arc::new(error)),
    }
}
    ...
    B: Buf,
{
    ...
    fn poll_open_bidi(&mut self, cx: &mut task::Context<'_>) -> Poll<Result<Self::BidiStream, StreamErrorIncoming>> {
        let bi = self
            .opening_bi
            .get_or_insert_with(|| Box::pin(stream::unfold(self.conn.clone(), |conn| async { Some((conn.open_bi().await, conn)) })));
    ...
    fn poll_open_send(&mut self, cx: &mut task::Context<'_>) -> Poll<Result<Self::SendStream, StreamErrorIncoming>> {
        let uni = self
            .opening_uni
            .get_or_insert_with(|| Box::pin(stream::unfold(self.conn.clone(), |conn| async { Some((conn.open_uni().await, conn)) })));
...
type ReadChunkFuture = ReusableBoxFuture<'static, (quinn::RecvStream, Result<Option<quinn::Chunk>, quinn::ReadError>)>;
...
pub struct SendStream<B: Buf> {
    ...
    stopped: Option<BoxFuture<'static, Result<Option<quinn::VarInt>, quinn::StoppedError>>>,
}
    ...
    B: Buf,
{
    ...
    fn poll_finish(&mut self, cx: &mut task::Context<'_>) -> Poll<Result<(), StreamErrorIncoming>> {
        let finish = Poll::Ready(self.stream.finish().map_err(|e| StreamErrorIncoming::Unknown(Box::new(e))));

        // Wait all data received by the peer
        let stopped = self.stopped.get_or_insert_with(|| Box::pin(self.stream.stopped()));
        ready!(stopped.poll_unpin(cx)).map_err(|e| StreamErrorIncoming::Unknown(Box::new(e)))?;
        self.stopped.take(); // Remove finish future
